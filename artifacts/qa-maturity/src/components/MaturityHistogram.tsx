/**
 * MaturityHistogram — гистограмма динамики зрелости команд за период.
 *
 * Показывает распределение количества команд по уровням зрелости (0/1/2/3)
 * в разрезе дат за выбранный период (Неделя/Месяц/Квартал).
 *
 * Данные: GET /api/metrics/history?period=week|month|quarter
 */
...
function formatDate(dateStr: string, period: "week" | "month" | "quarter"): string {
  ...
  // Квартал — показываем только день и месяц для экономии места
  return date.toLocaleDateString("ru-RU", { day: "numeric", month: "short" });
}
...
export function MaturityHistogram({ data, isLoading }: MaturityHistogramProps) {
        ...
        >
          ...
          <div style={{ display: 'grid', gap: `${Math.round(fontSize * 0.3)}px` }}>
            {payload.map((p: any, idx: number) => (
              <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', gap: `${Math.round(fontSize * 0.8)}px` }}>
                <span style={{ color: p.color, fontWeight: 500 }}>{p.name}</span>
                <span style={{ color: 'hsl(var(--foreground))', fontWeight: 'bold' }}>{p.value} ком.</span>
              </div>
            ))}
          </div>
          <p style={{
            ...
            Всего команд: {payload[0]?.payload?.teamCount ?? 0}
          </p>
        </div>
      );
    }
