 ...
 *
 * Использует AuthContext для проверки прав:
 *   canChangeStatus(teamId) — изменение статуса оценки (reviewer, manager своих команд, admin)
 */
...
export function TeamDashboard({ teamId }: { teamId: number }) {
  ...
  const canStatus = canChangeStatus(teamId);
    ...
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="max-w-[1800px] mx-auto pb-20">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    ...
                    className={cn("flex items-center gap-1.5 px-3 py-1 rounded-full border text-xs font-semibold transition-all hover:opacity-80", statusConfig.color)}
                  >
                    {isUpdatingStatus ? <Loader2 className="w-3 h-3 animate-spin" /> : statusConfig.label}
                ...
                <DropdownMenuContent align="start" className="bg-card border-border/60 shadow-xl shadow-black/30">
                  {ASSESSMENT_STATUSES.map((s) => (
                    <DropdownMenuItem
                      ...
                      className={cn("text-xs font-medium cursor-pointer gap-2", team.assessmentStatus === s.value ? "bg-primary/10 text-primary" : "")}
                    >
      ...
      <div className="space-y-12">
        {Object.entries(skillsByCategory).map(([category, skills], i) => (
          <motion.div key={category} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
            <div className="flex items-center gap-4 mb-6">
              <h3 className="text-xl font-display font-bold text-foreground/90 whitespace-nowrap">{category}</h3>
